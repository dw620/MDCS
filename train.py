...
optimizer = optim.Adam(model.parameters(), lr=3e-4, amsgrad=True, weight_decay=0.00001)
# optimizer = optim.Adam(model.parameters(), lr=3e-4, amsgrad=True, weight_decay=0.000005)
def train_and_valid(model, loss_function, optimizer, epochs=25):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")  # 若 gpu可用则用 gpu
    record = []
    best_acc = 0.0
    best_epoch = 0
    ...
    for epoch in range(epochs):
        ...
        for i, (inputs, labels) in enumerate(train_datas):  # train_data
        ...
        with torch.no_grad():
            ...
            valid_datas = tqdm(valid_data)

            for j, (inputs, labels) in enumerate(valid_datas):  # valid_data
                inputs = inputs.to(device)
                labels = labels.to(device)
        ...
        if avg_valid_acc > best_acc:
            ...
            best_epoch = epoch + 1
            torch.save(model, 'new_model_21.pth')
            torch.save(model.state_dict(), 'new_model_19_onlyweigths.pth')
